...
import { Search, MapPin, Phone, Calendar, Clock, PlusCircle, History, User, X } from 'lucide-react';
...
const PartiesComponent = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setRemarks(''); // Clear remarks, keep date maybe?
            setMeetingDate('');
    ...
    return (
        <div className="h-full w-full flex flex-col md:flex-row gap-4 overflow-hidden">
            ...
            <div className="w-full md:w-1/4 min-w-[250px] bg-white border border-slate-200 rounded-xl shadow-sm flex flex-col">
                <div className="p-4 border-b border-slate-100 bg-slate-50/50 rounded-t-xl">
                    ...
                    <div className="mt-3 relative">
                        <button
                            ...
                            className="absolute right-0 top-0 -mt-10 p-1.5 bg-blue-100 text-blue-600 rounded-lg hover:bg-blue-200 transition-colors"
                            title="Add Party"
                        >
                            <PlusCircle size={16} />
                        </button>
                ...
                {selectedParty ? (
                    <div className="flex-1 p-6 overflow-y-auto">
                        ...
                        <div className="bg-slate-50 p-4 rounded-lg border border-slate-100 mb-6">
                            <h3 className="text-lg font-bold text-slate-700">{selectedParty.name}</h3>
                            <div className="flex gap-4 mt-2 text-sm text-slate-600">
                                {selectedParty.phone && <span className="flex items-center gap-1"><Phone size={14} /> {selectedParty.phone}</span>}
                                {selectedParty.location && <span className="flex items-center gap-1"><MapPin size={14} /> {selectedParty.location}</span>}
                            </div>
                            <div className="mt-2 text-xs font-semibold text-slate-500 uppercase tracking-wide">
                                Balance: <span className="text-slate-800">${selectedParty.currentBalance?.toLocaleString() || 0}</span>
                            </div>
                        </div>
            ...
            {/* 🟦 COLUMN 3: History */}
            <div className="w-full md:w-1/3 min-w-[300px] bg-white border border-slate-200 rounded-xl shadow-sm flex flex-col">
            ...
            <AddPartyModal
                ...
                isSubmitting={isPartySubmitting}
            />
        </div >
    );
};
